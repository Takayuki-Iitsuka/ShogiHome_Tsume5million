...
# Bun を使う場合
bun run usi-csa-bridge:install
```
...
ShogiHomeの画面からコピーするのが簡単ですが、自分でエンコードする場合は以下の要領で変換します。
...
Bun 評価時も npm と同じ script 名を使います。

```
bun run usi-csa-bridge:build
./src/tests/e2e/usi-csa-bridge/run 1
```
